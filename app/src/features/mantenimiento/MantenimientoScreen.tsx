...
/**
 * Orquestación: el km actual del vehículo NUNCA se recalcula aquí — se pide
 * prestado a domain/estadisticas (que a su vez lo deriva de domain/viajes,
 * la única fuente de verdad real). Este patrón es el mismo que coordina
 * viajes + jornada en ViajesScreen (ver D-10).
 */
export function MantenimientoScreen() {
  ...
  return (
    <section className="pantalla">
      <h1 className="titulo-pantalla">Mantenimiento</h1>
      <p className="texto-mute" style={{ marginBottom: 16 }}>
        Km actual del vehículo (según viajes registrados): <strong>{kmActual.toFixed(0)} km</strong>
      </p>
          ...
          {listaAlertas.map(({ item, kmFaltantes, diasFaltantes, vencido, proximoAVencer }) => (
            <li key={item.id} className="tarjeta-viaje" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 6 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span className={claseParaEstado(vencido, proximoAVencer)}>
                  {textoParaEstado(kmFaltantes, diasFaltantes)}
                </span>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button
                  ...
                  onClick={() => {
                    ...
                    // Fase 13 (continuación): mismo criterio que viajes/jornada —
                    // intento de sincronización inmediato, best-effort.
                ...
                {item.origen === 'personalizado' && (
                  <button type="button" onClick={() => void eliminarItem(item.id)}>
                    Eliminar
                  </button>
                )}
              </div>
      ...
      <div className="tarjeta-viaje" style={{ flexDirection: 'column', gap: 8, alignItems: 'stretch' }}>
