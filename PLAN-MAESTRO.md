...
**Segundo candidato, encontrado revisando `domain/viajes/store.ts` (tampoco confirmado todavía, ver más abajo):** en `iniciarViaje()`, la llamada `seguidorActivo = await iniciarSeguimientoGPS(...)` **no está envuelta en try/catch**, y el estado `viajeEnCurso` se pone ANTES de esa llamada — así que si `iniciarSeguimientoGPS` (que termina llamando a `GpsTracking.startTracking()`, la que debería disparar el diálogo de permiso) falla o lanza un error en cualquier punto de la cadena, la UI de todos modos muestra el viaje como iniciado (coincide con la captura de pantalla: "Uber 0.0 km reales $20.000" apareció igual). Es decir: **es muy posible que el error SÍ esté pasando, pero nadie lo ve** — ni el usuario, ni nosotros, porque no hay ningún catch que lo muestre en pantalla ni lo loguee. Sin logs del dispositivo (logcat), no se puede confirmar si el fallo real está en `suscribirsePuntosGps` (que se llama ANTES que `startTracking`, en `gps.ts`), en el propio `startTracking`, o en otro punto.

**Corrección pendiente para la próxima sesión, antes de seguir adivinando a ciegas:** envolver esa llamada en try/catch y mostrar el error de forma visible en la pantalla (no silenciarlo) — así la próxima prueba en el dispositivo real va a decir exactamente qué está fallando, en vez de fallar en silencio. No se aplicó todavía porque el usuario prefirió dejarlo anotado y seguir probando otras cosas primero (backoff/mantenimiento/estadísticas/cuenta).

**Sigue pendiente, en orden:**

1. Agregar manejo de errores visible alrededor de `iniciarSeguimientoGPS` en `store.ts` (o en `ViajesScreen.tsx`, donde se llama a `iniciarViaje`) — prioridad #1 antes de seguir adivinando causas.
