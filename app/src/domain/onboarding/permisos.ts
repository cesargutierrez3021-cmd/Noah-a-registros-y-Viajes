...
import { solicitarPermisosUbicacion, solicitarIgnorarOptimizacionBateria as solicitarIgnorarOptimizacionBateriaGps } from '../viajes/gpsBackground'
import { solicitarPermisoBurbuja } from '../viajes/burbuja'
import { pedirPermisoVoz } from '../conversacion/voz'

/**
 * Onboarding (2026-09-15, pedido explícito del usuario): los permisos que la
 * app necesita, pedidos uno por uno apenas se abre por primera vez — antes
 * de mostrar ningún panel. Cada función de acá reutiliza el mismo puente
 * nativo que ya usa el resto de la app (D-18: no se duplica ninguna lógica
 * de permisos, solo se llama en un momento distinto):
 ...
 *   - batería (2026-09-15, bug real corregido: "la mayoría de los viajes
 *     queda en cero kilómetros") → GpsTrackingPlugin.solicitarIgnorarOptimizacionBateria()
 *     — sin esto, varios fabricantes (Xiaomi/Samsung/Huawei/Oppo) pueden
 *     parar la captura de GPS en segundo plano en silencio aunque el
 *     foreground service siga vivo.
 *
 * Ninguno de estos pasos rechaza si el usuario niega — el onboarding sigue
...
export async function solicitarIgnorarOptimizacionBateria(): Promise<boolean> {
  if (!esAndroidNativo()) return true
  try {
    return await solicitarIgnorarOptimizacionBateriaGps()
  } catch {
    return false
  }
}
